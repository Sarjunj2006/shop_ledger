...
document.getElementById("owner-logout-btn").addEventListener("click", async () => {
  ...
  setOwnerMode(false);
  // Send the view back to New Entry, since owner-only tabs are no longer reachable.
  switchToTab("entry");
});
...
async function api(path, opts = {}) {
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    if (res.status === 401) {
      // Token missing/expired (e.g. server restarted) — drop back to staff view.
      sessionStorage.removeItem(OWNER_TOKEN_KEY);
      setOwnerMode(false);
    }
